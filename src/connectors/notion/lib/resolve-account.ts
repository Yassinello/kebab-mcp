/**
 * Phase 74 (MATL-03/04): shared account-resolution helper for the 5 Notion
 * tool handlers. Wraps `resolveConnectorAccount("notion", …)` and converts a
 ...
 * thread into the notion-api layer. On `error_no_account` we surface a
 * "not configured" message; on `error_account_required` we list the
 * available account NAMES so the caller knows what to pass.
 */
...
export type NotionAccountResult =
...
/**
 * Resolve the Notion account for a tool call. `account` is the optional
 * selector from the tool params (name or slug). `exactOptionalPropertyTypes`:
 * only pass `account` when defined.
 */
export async function resolveNotionTokens(account?: string): Promise<NotionAccountResult> {
  ...
  if (resolution.error === "error_no_account") {
    return {
      ok: false,
      result: errorResult(
        "Notion pack is not configured. Add NOTION_API_KEY to your environment variables."
      ),
    };
  }
  ...
  return {
    ok: false,
    result: errorResult(
      `Multiple Notion accounts are connected. Re-run with the "account" parameter set to one of: ${resolution.available_accounts.join(", ")}.`
    ),
  };
}
